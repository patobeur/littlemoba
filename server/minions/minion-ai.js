...
class MinionAI {
		...
		if (targetInfo.target) {
			...
			if (distance > hitDistance) {
				...
				const timeSinceLastAttack =
					(now - (minion.lastAttackTime || 0)) / 1000;
				...
				if (timeSinceLastAttack >= autoAttackCd) {
					...
					const dz =
						(targetInfo.target.z || targetInfo.target.y) - minion.z;
			...
			const enemyBase =
				minion.faction === "blue"
					? structures.BaseTeamB
					: structures.BaseTeamA;
			if (enemyBase && !enemyBase.isDead) {
				this.moveTowards(
					minion,
					{ x: enemyBase.x, z: enemyBase.y },
					speed,
					dt,
					allMinions
				);
	...
	static findTarget(
		...
		if (minion.targetId && minion.targetType) {
			const currentTarget = this.getTargetById(
				minion.targetId,
				minion.targetType,
				allMinions,
				players,
		...
		for (const otherMinion of allMinions) {
			if (
				otherMinion.id === minion.id ||
				otherMinion.faction === minion.faction ||
				otherMinion.isDead
			)
				continue;
	...
	static calculateAvoidanceForce(minion, allMinions, desiredDir) {
		...
		const AVOIDANCE_STRENGTH =
			GAME_CONSTANTS.MINION_AVOIDANCE_STRENGTH || 0.5;
		const TANGENTIAL_STRENGTH =
			GAME_CONSTANTS.MINION_TANGENTIAL_STRENGTH || 0.1; // Force to go around
